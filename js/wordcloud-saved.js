...
class Wordcloud {
    ...
    // this.start = d3.layout.cloud().size([W, H])
    // .words(words)
...
.await(function(error, recipe1_texture, recipe1_review, recipe2_texture, recipe2_review) {
    if (error) {
        ...
        console.log("--- same textures ---");
        console.log(same_textures);

        // let same_recipe1_texture = _.chain(recipe1_texture)
        //                             .filter(function(d){ if(same_textures.indexOf(d.word) >= 0) return d })
        //                             .sortBy(['word'])
        //                             .value();
        // recipe1_texture = _.chain(recipe1_texture)
        //                       .filter(function(d){ return same_textures.indexOf(d.word) == -1;})
        //                       .concat(same_recipe1_texture) 
        //                       .reverse()
        //                       .value();
        //
        //
        // let same_recipe2_texture = _.chain(recipe2_texture)
        //                             .filter(function(d){ if(same_textures.indexOf(d.word) >= 0) return d })
        //                             .sortBy(['word'])
        //                             .value();
        // recipe2_texture = _.chain(recipe2_texture)
        //                               .filter(function(d){ return same_textures.indexOf(d.word) == -1;})
        //                               .concat(same_recipe2_texture) 
        //                               .reverse()
        //                               .value();
        
        // same_texturesと色の対応を作る
